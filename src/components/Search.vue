...
const keyword = ref(localStorage.getItem("keyword") ?? "Batman");
...
<template>
  <form @submit.prevent="handleSearch">
    <div class="main-search relative text-gray-600">
      <input type="text" placeholder="Search any movies . . . ." class="
          w-full
          px-14
          py-3
          rounded-md
        ...
        " v-model="keyword" @keydown.enter.prevent="handleSearch" />
    </div>
  </form>
  ...
  <div class="mt-3 md:mt-5" v-if="isRecent">
    <div class="flex items-center">
      ...
      <div class="flex items-center text-white-100 mx-1 my-2  cursor-pointer" @click="clearRecent">
    ...
    <div class="flex mt-3 flex-wrap h-12 overflow-auto">
      <div class="flex items-center m-1 text-zinc-100/20 md:m-2" v-for="recent in recents" :key="recent">
        <span class="tracking-wide cursor-pointer md:text-lg" @click="changeKeyword">{{ recent }}</span>

        <svg class="w-4 h-4 ml-2 md:w-5 md:h-5 md:ml-3 cursor-pointer" viewBox="0 0 24 24" fill="none"
          xmlns="http://www.w3.org/2000/svg" @click="filterRecent($event)" :data-recent="recent">
          <g id="Iconly/Broken/Close Square">
            <g id="Close Square">
              <path id="Close Square_2" fill-rule="evenodd" clip-rule="evenodd"
